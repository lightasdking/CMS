...
const TeacherListPage = async ({
  ...
  const renderRow = (item: TeacherList) => (
    ...
    >
      ...
      <td className="hidden md:table-cell">{item.username}</td>
      <td className="hidden md:table-cell">
        {item.subjects.length > 0 ? item.subjects.map((subject) => subject.name).join(",") : "-"}
      </td>
      <td className="hidden md:table-cell">
        {item.classes.length > 0 ? item.classes.map((classItem) => classItem.name).join(",") : "-"}
      </td>
      ...
      <td>
        <section className="inline-flex items-center gap-2">
          <Link href={`/list/teachers/${item.id}`}>
            <button className="w-7 h-7 inline-flex items-center justify-center rounded-full bg-lamaSky">
              <Image src="/view.png" alt="" width={16} height={16} />
            </button>
          </Link>
          {role === "admin" ? (
  ...
  const fetchTeachers = async () => {
    ...
    const total = await prisma.teacher.count({ where: query });
    return { teachers, total };
  };

  const { teachers: data, total: count } = await fetchTeachers();

  const renderHeader = () => (
    <header className="inline-flex items-center justify-between w-full">
      <h1 className="hidden md:inline text-lg font-semibold">All Teachers</h1>
      <section className="flex flex-col md:inline-flex md:flex-row items-center gap-4 w-full md:w-auto">
        ...
        <div className="inline-flex items-center gap-4 self-end">
          ...
          {role === "admin" ? <FormContainer table="teacher" type="create" /> : null}
        </div>
      </section>
    </header>
  );
