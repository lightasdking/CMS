    ...
    items: [
      {
        icon: "/home.png",
        label: "Home",
        href: "/(dashboard)/admin", // Will be dynamically updated based on role
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        icon: "/logout.png",
        label: "Logout",
        href: "/sign-in", // Will redirect to sign-in page
        visible: ["admin", "teacher", "student", "parent"],
      },
    ],
  },
  ...
  useEffect(() => {
    ...
    if (isLoaded && user) {
      ...
      if (userRole) {
        ...
        // Update home link to point to user's dashboard
        const homeMenuItem = menuItems[0].items[0];
        if (homeMenuItem) {
          homeMenuItem.href = `/(dashboard)/${userRole}`;
        }
      }
    }
  }, [isLoaded, user]);
      ...
      {menuItems.map((i) => (
        <div className="flex flex-col gap-2" key={i.title}>
          ...
          {i.items.map((item) => {
            ...
            return null; // Important to return null for items that shouldn't be shown
          })}
        </div>
      ))}
    </div>
